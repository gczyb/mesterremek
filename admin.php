...
if (!$user || !isset($user['admin']) || $user['admin'] != 1) {
    ...
    die("<div class='forbidden-container'>
            <h1>403 Forbidden</h1><p>You do not have permission to access this dashboard.</p>
            <a href='index.php' class='forbidden-link'>Return Home</a>
         </div>");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_record') {
        ...
        $name = $_POST['name']; 
        $type = $_POST['weapon_type']; 
        $atk = (int)$_POST['atk'];
        $hit = (int)$_POST['hit_rate']; 
        $crit = (int)$_POST['crit_rate']; 
        $wt = (int)$_POST['weight'];
        $min_r = (int)$_POST['min_range']; 
        $max_r = (int)$_POST['max_range']; 
        $dur = (int)$_POST['durability'];
        $desc = $_POST['description']; 
        $img = handleAdminUpload($_FILES['image'] ?? null, 'weapons');
        ...
        $ally = (int)$_POST['ally']; 
        $hp = (int)$_POST['base_hp']; 
        $str = (int)$_POST['base_str'];
        $dex = (int)$_POST['base_dex'];
        $skill = (int)$_POST['base_skill']; 
        $def = (int)$_POST['base_def'];
        $luck = (int)$_POST['base_luck']; 
        $move = (int)$_POST['base_move']; 
        $desc = $_POST['description'];
...
<body class="admin-page">
    <nav class="nav">
        ...
        <div class="mobile-menu" id="mobileMenu">
            ...
            <a href="wiki.php">Wiki</a>
            <?php if ($user): ?>
                <a href="admin.php" class="admin-link-mobile">Admin Dashboard</a>
                <a href="logout.php">Logout</a>
            <?php endif; ?>
        </div>
    </nav>
    ...
    <div class="admin-container">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group form-full">
                        ...
                        <?php if(!empty($edit_data['image_url'])): ?>
                            <img src="<?php echo $edit_data['image_url']; ?>" class="admin-header-img"><br>
                        <?php endif; ?>
                    ...
                    <div class="form-group form-full">
                        ...
                        <div class="form-help-text">
                            <a href="https://www.phpbb.com/community/help/bbcode#f1r0" class="form-help-link">Help</a>
                        </div>
                    </div>
                </div>
        ...
        <div class="card">
            <div class="admin-list-header">
                <h3 class="admin-list-title">Registered Users</h3>
                
                <form method="GET" action="admin.php" class="admin-search-form">
                    <input type="hidden" name="tab" value="users">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search username or email..." class="admin-search-input">
                    <button type="submit" class="btn btn-search">Search</button>
                    <?php if ($search !== ''): ?>
                        <a href="admin.php?tab=users" class="btn btn-outline btn-clear">Clear</a>
                    <?php endif; ?>
                </form>
            </div>
            ...
            <table class="data-table">
                    ...
                    <tr>
                        ...
                        <td data-label="Actions">
                            <div class="action-buttons-row">
                                ...
                                <form method="POST" onsubmit="return confirm('Delete user?');" class="inline-form">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="table-empty-state">No users found matching "<?php echo htmlspecialchars($search); ?>"</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <?php if(!empty($edit_data['image_url'])): ?>
                            <img src="<?php echo $edit_data['image_url']; ?>" class="admin-preview-img"><br>
                        <?php endif; ?>
                        <input type="file" name="image" accept="image/*">
                    </div>
                    <div class="form-group form-full">
                ...
                <button type="submit" class="btn btn-submit">Save Class</button>
            </form>
        </div>
        ...
        <div class="card">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label>Type (Sword, Bow, etc)</label>
                        <input type="text" name="weapon_type" value="<?php echo htmlspecialchars($edit_data['weapon_type'] ?? ''); ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Attack (Mt)</label>
                        <input type="number" name="atk" value="<?php echo $edit_data['atk'] ?? 0; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Hit Rate %</label>
                        <input type="number" name="hit_rate" value="<?php echo $edit_data['hit_rate'] ?? 90; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Crit Rate %</label>
                        <input type="number" name="crit_rate" value="<?php echo $edit_data['crit_rate'] ?? 0; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Weight (Wt)</label>
                        <input type="number" name="weight" value="<?php echo $edit_data['weight'] ?? 5; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Min Range</label>
                        <input type="number" name="min_range" value="<?php echo $edit_data['min_range'] ?? 1; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Max Range</label>
                        <input type="number" name="max_range" value="<?php echo $edit_data['max_range'] ?? 1; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Durability</label>
                        <input type="number" name="durability" value="<?php echo $edit_data['durability'] ?? 30; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Icon Image</label>
                        <?php if(!empty($edit_data['image_url'])): ?>
                            <img src="<?php echo $edit_data['image_url']; ?>" class="admin-preview-icon"><br>
                        <?php endif; ?>
                        <input type="file" name="image" accept="image/*">
                    </div>
                    <div class="form-group form-full">
                ...
                <button type="submit" class="btn btn-submit">Save Weapon</button>
            </form>
        </div>
        ...
        <div class="card">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label>Base HP</label>
                        <input type="number" name="base_hp" value="<?php echo $edit_data['base_hp'] ?? 20; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Strength (Str)</label>
                        <input type="number" name="base_str" value="<?php echo $edit_data['base_str'] ?? 5; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Dexterity (Dex)</label>
                        <input type="number" name="base_dex" value="<?php echo $edit_data['base_dex'] ?? 5; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Skill</label>
                        <input type="number" name="base_skill" value="<?php echo $edit_data['base_skill'] ?? 5; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Defense (Def)</label>
                        <input type="number" name="base_def" value="<?php echo $edit_data['base_def'] ?? 3; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Luck</label>
                        <input type="number" name="base_luck" value="<?php echo $edit_data['base_luck'] ?? 2; ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Movement (Move)</label>
                        <input type="number" name="base_move" value="<?php echo $edit_data['base_move'] ?? 5; ?>" required>
                    </div>
                    <div class="form-group form-full">
                        <label>Description</label>
                        <textarea name="description" rows="2"><?php echo htmlspecialchars($edit_data['description'] ?? ''); ?></textarea>
